...
class CreateVouchersTable extends Migration
{
    public function up()
    {
        Schema::create('vouchers', function (Blueprint $table) {
            ...
            // Purchase fields
            $table->foreignIdFor(config('discounts.model_class.customer'));
            $table->foreignIdFor(config('discounts.model_class.location'));
            $table->foreignIdFor(\Tipoff\Vouchers\Models\VoucherType::class);
            $table->foreignIdFor(config('discounts.model_class.order'), 'purchase_order_id');

            // Redemption fields
            $table->dateTime('redeemable_at'); // Defaults to 24 hours after created_at
            $table->foreignIdFor(config('discounts.model_class.order'));
